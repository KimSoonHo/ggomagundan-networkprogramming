...
test('GET', function(){
    ...
    jQuery.get('http://117.17.158.252/cgi-bin/timepro.cgi?tmenu=wirelessconf&smenu=info', function(res){
        ...
        // var indexCenter = htmlText.indexOf("menu_content_list_table");
        // var subText = htmlText.substr(indexCenter+2);
        // indexCenter = subText.indexOf("menu_content_list_table");
       	// subText = subText.substr(indexCenter);
       	// indexCenter = subText.indexOf("<table");
       	// subText = subText.substr(indexCenter);
       	// indexCenter = subText.indexOf("</table>");
       	// subText = subText.substr(0, indexCenter+8);
        
        
        //var regExp = /"<table class=\"menu_content_list_table\"(.+)>(.+)<\/table>"/gi;
        
       
        // var myArray =  htmlText.match(/class=\"menu_content_list_table\">([^&]+)<\/table>/g);
        var myArray =  htmlText.match(/class=\"menu_content_list_table\">([^]+)form>/g);
        
        var containText = myArray[0].substr(0,myArray[0].length-7); 

        
        var htmls = "<table "+ containText;
        
        $("#contain").html(htmls);
       // $("#contain").text(htmlText);
...
//         
    // //$('#who').css('height','280px');
		...
		var htmls ;
		if(hwang){
			htmls="<table><tr><td><img src='./images/hwang_here_1.png' /></td>";
		}else{
			htmls="<table><tr><td><img src='./images/hwang_empty_1.png' /></td>";
		}
		if(park){
